document.addEventListener('DOMContentLoaded', () => {
    ...
    if (addMemberBtn && membersContainer) {
        addMemberBtn.addEventListener('click', () => {
            ...
            const memberLabelIndex = slotIndex + 1; // "Member 1", "Member 2"
    ...
    function relabelMembers() {
        ...
        memberCards.forEach((card, index) => {
            ...
            if (badge) badge.innerText = `Member ${index + 1}`;
            // Also update naming attributes if we want strict ordering, 
            // but placeholders entry.MEMBER_X... handles it loosely.
        });
    }
    ...
    if (form) {
        ...
        // --- Hidden Iframe Handler ---
        const iframe = document.getElementById('hidden_iframe');
        let submitted = false;

        // When the form is submitted, we set a flag
